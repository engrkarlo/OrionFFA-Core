...
public final class GuiProtectionListener implements Listener {
    ...
    private boolean spectatorTabInstalled;
    ...
    public void drag(InventoryDragEvent event) {
        if(spectators.owns(event.getView().getTopInventory())||kitGuis.owns(event.getView().getTopInventory())||guis.owns(event.getView().getTopInventory())) { int top=event.getView().getTopInventory().getSize(); if(event.getRawSlots().stream().anyMatch(s->s<top)) event.setCancelled(true); return; }
    ...
    public void interact(PlayerInteractEvent event) {
        ...
        switch(lobbyMenus.action(item)){
            ...
            case "open_party" -> guis.openParty(player);
    ...
    public void join(PlayerJoinEvent event){
        ...
        installSpectateTabCompleter();
    }

    @EventHandler
    public void chat(AsyncPlayerChatEvent event){kitGuis.chat(event);}

    /** Wrap the existing command completer instead of replacing it, preserving every existing completion. */
    private void installSpectateTabCompleter(){
        if(spectatorTabInstalled)return;
        PluginCommand command=Bukkit.getPluginCommand("orionffa");
        if(command==null)return;
        TabCompleter delegate=command.getTabCompleter();
        if(delegate==null)return;
        command.setTabCompleter((sender, cmd, alias, args) -> {
            if(args.length==2 && args[0].equalsIgnoreCase("spectate")){
                List<String> values=new ArrayList<>();
                if("leave".startsWith(args[1].toLowerCase(Locale.ROOT)))values.add("leave");
                Bukkit.getOnlinePlayers().stream()
                        .filter(p->!p.equals(sender)&&spectators.isValidTarget(p))
                        .map(Player::getName)
                        .filter(n->n.toLowerCase(Locale.ROOT).startsWith(args[1].toLowerCase(Locale.ROOT)))
                        .sorted(Comparator.naturalOrder()).forEach(values::add);
                return values;
            }
            return delegate.onTabComplete(sender,cmd,alias,args);
        });
        spectatorTabInstalled=true;
    }
}