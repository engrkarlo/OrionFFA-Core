...
public final class FfaService {
    ...
    public ServiceResult editKit(Player player,String kitId) {
        ...
        PlayerSession session=sessions.enter(player); FfaState previousState=session.state(); String previousArena=session.arenaId(); if(!teleports.teleport(player,config.runtime().editKit())){if(previousState==FfaState.OUTSIDE)sessions.remove(player.getUniqueId());return ServiceResult.fail("world-unavailable");} if(previousArena!=null)arenas.leave(previousArena,player.getUniqueId()); applyKit(player,kit.get(),customKits.find(player.getUniqueId(),kit.get().id()).orElse(null)); session.kitId(kit.get().id());session.arenaId(null);session.spectatorTarget(null);session.state(FfaState.EDITING_KIT); return ServiceResult.ok("kit-editing",Map.of("kit",kit.get().id()));
    }
    ...
    public ServiceResult joinKitAt(Player player,String kitId,String arenaId){if(!config.runtime().ffaEnabled())return ServiceResult.fail("ffa-disabled");Optional<KitDefinition> k=kits.find(kitId);Optional<Arena> a=arenas.get(arenaId);if(k.isEmpty())return ServiceResult.fail("kit-unavailable");if(a.isEmpty()||resets.isResetting(a.get().id())||!arenas.availableFor(k.get().id()).contains(a.get()))return ServiceResult.fail("arena-unavailable");return join(player,k.get(),a.get());}
